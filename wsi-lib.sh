...
wsi_notify() {
  ...
  if command -v notify-send >/dev/null 2>&1; then
    notify-send -a wsi "$title" "$body" >/dev/null 2>&1 || true
  elif command -v gdbus >/dev/null 2>&1; then
    gdbus call --session --dest org.freedesktop.Notifications \
      --object-path /org/freedesktop/Notifications \
      --method org.freedesktop.Notifications.Notify \
      wsi 0 "" "$title" "$body" [] {} 3000 >/dev/null 2>&1 || true
  fi
}

wsi_note() {
  local title="$1" body="${2:-}" log_dir="${XDG_CACHE_HOME:-$HOME/.cache}"
  wsi_notify "$title" "$body"
  printf '%s%s%s\n' "$title" "${body:+: }" "$body" >&2
  mkdir -p "$log_dir" 2>/dev/null || true
  { printf '%s %s%s%s\n' "$(date '+%F %T')" "$title" "${body:+: }" "$body" >> "$log_dir/wsi-model.log"; } 2>/dev/null || true
}
...
wsi_prep_model_in_shm() {
  ...
  [[ -n "$model_on_disk" ]] || wsi_die "Model Not Set" "Set WMODEL or MODEL_ON_DISK"
  [[ -f "$model_on_disk" ]] || wsi_die "Model Not Found" "$model_on_disk"
  ...
  if [[ ! -f "$model_in_shm" ]]; then
    cp -f "$model_on_disk" "$model_in_shm" || wsi_die "Load Failed" "$model_name -> /dev/shm"
    wsi_note "Model Loaded" "$model_name -> /dev/shm"
  fi
  ...
  for model in ${WSI_MODELS:-}; do
    ...
    if wsi_other_jobs_active; then
      wsi_note "Swap Skipped" "$(basename "$stale_in_shm") still in /dev/shm; another job is active"
      continue
    fi
    rm -f -- "$stale_in_shm" || wsi_die "Remove Failed" "$(basename "$stale_in_shm") from /dev/shm"
    wsi_note "Model Removed" "$(basename "$stale_in_shm") from /dev/shm"
  done
...
wsi_to_wav16k_mono() {
  ...
  if command -v ffmpeg >/dev/null 2>&1; then
    ...
    wsi_die "Audio Tool Missing" "Install ffmpeg (recommended) or sox"
  fi
}
